...
**Known risk — this one fired during execution.** Task 6 Step 5 assumed `window.timerApp` would be assigned before Alpine initialises, on the reasoning that both scripts are deferred and run in document order. The reasoning about *ordering* was right; the conclusion was wrong, because the module sat at the end of `<body>` and Alpine's tag was in `<head>` — so Alpine ran **first** and threw `ReferenceError: timerApp is not defined`, plus a cascade of twelve more as every binding failed to resolve.

The documented fallback — registering via `document.addEventListener('alpine:init', ...)` — **also failed, for the same underlying reason.** Alpine's CDN build calls `Alpine.start()` synchronously on execution, and `start()` both fires `alpine:init` and walks the DOM. A listener added by a module that runs afterwards is always too late. `alpine:init` is only usable from a script that executes *before* Alpine's.

The change that worked: move `<script type="module" src="static/js/timer.js">` into `<head>`, **above** the Alpine tag, and keep `window.timerApp = timerApp` with `x-data="timerApp()" x-init="init()"` unchanged. Deferred scripts do execute in document order, so putting the module first is what actually delivers the guarantee the original step was reaching for.

Why this ordering worked before the refactor: `timer.js` was a classic, non-deferred script at the end of `<body>`, so it executed *during* parsing — before any deferred script. Converting it to a module made it deferred and silently moved it after Alpine. Both tags now carry a comment saying not to reorder them.

**Lesson for later phases.** Phase 5 vendors Alpine locally (D3). When that happens, re-verify this ordering rather than assuming a local `<script>` behaves like the CDN one.
