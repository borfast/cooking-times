...
*As of Phase 4 the second and fourth no longer hold unconditionally, and as of 2026-07-27 nor does the first.* A dish has two phases — `startTime → heatOffTime` on the heat, then `restSeconds` off it, ready at `finishTime` — so `totalTime = max(cookDuration + restSeconds)`. Capacity and changeover time are declared per kitchen, and the synchronised finish is now conditional: when the kitchen cannot support it, the chosen strategy either reports the conflict, moves dishes earlier so they keep warm, or moves them later so the meal is ready later. Only the time on the heat counts against capacity. A dish may also carry a signed serve offset, so `mealTime = max(cookDuration + restSeconds - serveOffset)` and each dish finishes at `mealTime + its own offset`; with every offset at zero this is exactly the simple synchronised finish.
...
The scheduler will happily tell you to have six things cooking at once. Real kitchens have four hobs, one oven at one temperature, and a finite number of pans. This is the single largest gap between the schedule the app produces and a schedule you can actually execute, and closing it means modelling resources — which is a substantially different product, not a patch. *Built in Phase 4, then removed on the user's decision (2026-07-26): capacity settings, conflict warnings and the two rescheduling strategies were all deleted in favour of the original app's single simple timing. **This gap is open and deliberately accepted.** The commit that removed it is `ace99c6`, so the work is recoverable if the decision is ever revisited. What was learned and is worth keeping if so: overlap is invariant under the common finish, so any resolution must break the synchronised finish; and a greedy placer can only stagger dishes earlier as far as t=0, which bounds what that strategy can ever achieve.*

**G14 — Zero prep and transition time.** Items scheduled two seconds apart are assumed startable two seconds apart. In practice each start costs you draining, seasoning, or finding the lid. *Built in Phase 4 alongside G13 and removed with it on the same decision. **Open and deliberately accepted.***
...
**G20 — No tests, no linting, no CI.** For 2,000 lines this is defensible; for the scheduling and recalculation logic specifically it is not. `recalculateSchedulePreservingProgress` has at least six meaningful cases and none of them are pinned. *Closed: Phase 1 added the test runner, and the suite now stands at 103 unit tests over the whole core. Biome provides linting and formatting, wired into `npm run lint` and run automatically before `npm test`; GitHub Actions runs lint plus tests on Node 24 and latest, with CodeQL alongside. Biome itself is the only dependency and is dev-only — the shipped app is still dependency-free with no build step.*
...
No quantity, no thickness, no cooking method, no starting temperature. One potato and two kilos of potatoes get 1200 seconds. A steak's cook time depends far more on thickness than on the doneness the app does model. *Closed as designed (2026-07-27). The app deliberately does not model quantity, thickness or method: doing so means inventing coefficients, and a fabricated number presented as arithmetic is worse than a fixed one the cook can see is approximate. Instead it stops claiming authority — any dish's time can be corrected and the correction is remembered per food-and-option. Reviewed with the user, who confirmed the override is the right answer rather than a shortfall.*
...
There is no way to say "the steak should rest for five minutes", "serve the soup first", or "keep the potatoes warm". Resting time in particular is standard practice for exactly the meat this app schedules, and the model has no slot for it. *Closed. Resting landed in Phase 4; serve order followed on 2026-07-27. A dish now has two phases — on the heat, then resting off it — so resting meat lands with everything else, a resting dish occupies no burner for capacity purposes, and the timer announces both putting it on and taking it off. Per-dish serve offsets were excluded by D4 at the time and added later: a dish may be served a chosen number of minutes before or after the rest, which is what covers 'serve the soup first'. The schedule therefore has two moments — the meal, when offset-0 dishes land, and the total, the end of the timeline. Keeping warm is still the cook's problem; the app only tells you when things are ready.*
