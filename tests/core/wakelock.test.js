...
test('a release during an in-flight request does not leave the lock held', async () => {
  ...
  let resolveRequest;
  const acquired = { released: false, release: async () => { acquired.released = true; } };
  const navigatorLike = {
    wakeLock: { request: () => new Promise((resolve) => { resolveRequest = () => resolve(acquired); }) },
  };
  const lock = createWakeLock(navigatorLike);
  ...
  assert.equal(lock.isHeld(), false, 'lock was resurrected by the late request');
