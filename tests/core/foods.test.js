...
test('resolveSelection returns null for an unknown food', () => {
    assert.equal(
        resolveSelection(catalogue, { itemId: 'i1', foodId: 'nope' }),
        null,
    );
});
...
test('catalogueProblems reports every kind of malformation', () => {
    const problems = catalogueProblems([
        ...
        },
        {
            ...
            name: 'B',
            category: 'Meat',
            options: [{ id: 'o', label: 'O', seconds: 0 }],
        },
        {
            id: 'c',
            name: 'C',
    ...
    assert.ok(
        problems.some((problem) => problem.includes('duplicate food id')),
    );
...
test('the shipped foods.json is well-formed', () => {
    const raw = readFileSync(
        new URL('../../static/foods.json', import.meta.url),
        'utf8',
    );
...
test('the shipped catalogue exercises one, two and three option foods', () => {
    ...
    const counts = new Set(
        JSON.parse(raw).foods.map((food) => food.options.length),
    );
...
test('resolveSelection carries the food rest, defaulting to zero', () => {
    ...
    assert.equal(
        resolveSelection(withRest, { itemId: 'i1', foodId: 'beef-steak' })
            .restSeconds,
        300,
    );
    assert.equal(
...
test('the shipped catalogue rests meat and nothing else', () => {
    ...
    const resting = foods
        .filter((food) => food.restSeconds > 0)
        .map((food) => food.id)
        .sort();
